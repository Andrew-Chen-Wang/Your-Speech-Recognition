...
class Audio(object):
    """Streams raw audio from microphone. Data is received in a separate thread, and
    stored in a buffer, to be read from."""
    ...
    def __init__(self, callback=None, device=None, input_rate=RATE_PROCESS, file=None):
        def proxy_callback(in_data, *_args):  # frame_count, time_info, status
...
class VADAudio(Audio):
    ...
    def vad_collector(self, padding_ms=300, ratio=0.75, frames=None):
        ...
        utterance, separated by yielding a single None. Determines voice activity by
        ratio of frames in padding_ms. Uses a buffer to include padding_ms prior to
        being triggered.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Set aggressiveness of VAD: an integer between 0 and 3, 0 being the "
        "least aggressive about filtering out non-speech, 3 the most aggressive. "
        "Default: 1",
    )
    parser.add_argument("--nospinner", action="store_true", help="Disable spinner")
    parser.add_argument(
        "-w", "--savewav", help="Save .wav files of utterances to given directory"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--model",
        required=True,
        help="Path to the model (protocol buffer binary file, or entire directory "
        "containing all standard-named files for model)",
    )
    ...
    parser.add_argument(
        ...
        help=f"Input device sample rate. Default: {DEFAULT_SAMPLE_RATE}. Your device "
        f"may require 44100.",
    )
